...
# Writable directory; on Railway Hobby this is just ./reports, on Pro set DATA_DIR=/data
...
def ensure_chart_generated(timeframe: str = "YTD"):
    gen = ROOT / "chart_generator.py"  # or pnl_plotly.py if that's your filename
    if not gen.exists():
...
async def report_detail(request: Request, slug: str):
    ...
    # Decide mount & compute RELATIVE path for the mount (fixes /files/reports/reports/*.pdf 404)
    try:
    ...
    if in_data_dir:
        base_prefix = "/files"
        rel_path = file_path.relative_to(DATA_DIR).as_posix()   # <-- key fix
    else:
        base_prefix = "/assets"
    ...
    if ext in {".html", ".pdf"}:
        embed_src = f"{base_prefix}/{rel_path}"
        return templates.TemplateResponse(
            "report_detail.html",
...
def chart_redirect():
    chart_path = DATA_DIR / "pnl_dashboard.html"
    if chart_path.exists():
        return RedirectResponse(url="/files/pnl_dashboard.html")
    # fallback (committed asset)
...
async def partial_chart(request: Request, tf: str = "YTD"):
    ...
    html = f"""
    <div id="chart" class="rounded-2xl border border-neutral-900 bg-neutral-900/60 p-2 shadow-[0_10px_30px_rgba(0,0,0,0.35)]">
